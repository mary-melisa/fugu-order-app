 ...
 */
   ...
   *   isLittleEndian    是否小端
   *   length            长度
   * 为了不影响其它地方,专门为游客新建的一个方法
   * @returns {*}
   */
  visitorUIntToByteArray(value, isLittleEndian, length) {
    let dataView = new DataView((new Uint8Array(length)).buffer);
    switch (length) {
      case 8:
        // 账户编号，8 Byte double 类型
        dataView.setBigUint64(0, value, isLittleEndian);
        break;
      default:
        return {};
    }
  ...
  byteArrayToUInt(array, isLittleEndian) {
    ...
    switch (array.length) {
      ...
      case 4:
        ...
        return value;
      case 8:
        value = dataView.getBigUint64(0, isLittleEndian);
        return value;
      default:
        return 0;
    }
